...
#### ***Тест***  
```bash
    k0pec@upc:~/old_playbook/java $ molecule test
...
```bash
    k0pec@upc:~/elastic-role $ molecule init scenario -d docker
    INFO     Initializing new scenario default...
    INFO     Initialized scenario in /home/k0pec/elastic-role/molecule/default successfully.
```

#### ***Добавлены Ubuntu:latest, centors:7 , тест запущен***
```bash
    k0pec@upc:~/elastic-role $ molecule test
    INFO     default scenario test matrix: dependency, lint, cleanup, destroy, syntax, create, prepare, converge, idempotence, side_effect, verify, cleanup, destroy
    INFO     Performing prerun...
...
#### ***Cоздание роли kibana_role***
```bash
    k0pec@upc:~$ molecule init role -d docker kibana-role
    INFO     Initializing new role kibana-role...
    Using /etc/ansible/ansible.cfg as config file
    - Role kibana-role was created successfully
    INFO     Initialized role in /home/k0pec/kibana-role successfully.
```


#### ***Тестирование роли***
```bash
    k0pec@upc:~/kibana-role$ molecule test
    INFO     default scenario test matrix: dependency, lint, cleanup, destroy, syntax, create, prepare, converge, idempotence, side_effect, verify, cleanup, destroy
    INFO     Performing prerun...
...
```yaml
---
  - src: git@github.com:k0pec/kibana-role.git
    scm: git
    version: "1.0.1"
    name: kibana-role 
  - src: git@github.com:k0pec/elastic-role.git
    scm: git
    version: "1.0.1"
    name: elastic-role
  - src: git@github.com:k0pec/java-role.git
    scm: git
    version: "1.0.1"
    name: java-role 
```
...
#### ***playbook***  
```bash
    k0pec@upc:~/playbook$ ansible-playbook -i inventory/prod.yml site.yml
